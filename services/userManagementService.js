...
const createUserAccount = async (name, phoneNumber, pin) => {
  try {
    ...
    return {
      success: true,
      message: `User registered successfully with transaction hash: ${tx.hash}`,
      userDetails: { name, phoneNumber, pin, transactionHash: tx.hash },
    };
  } catch (error) {
    console.error('Error during registration:', error);
    throw new Error('Failed to register user on the blockchain.');
...
const isUserRegistered = async (phoneNumber) => {
  try {
    const userDetails = await userManagementContract.getUserInfo(phoneNumber);
    return userDetails.walletAddress !== ethers.constants.AddressZero;
  } catch (error) {
    console.error('Error checking user registration:', error);
    return false;
  }
...
const loginUser = async (phoneNumber, pin) => {
  try {
    ...
    if (!userDetails || userDetails.pin !== pin) {
